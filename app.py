...
st.set_page_config(page_title="MedBot Intelligence OS", layout="wide", page_icon="🛸")
...
st.markdown("""
    ...
    .stButton>button {
        ...
        color: white; border: none; border-radius: 10px; padding: 10px 25px; font-size: 18px;
    }
    </style>
...
def get_vector_store():
    ...
    # تأكد إن أسماء الملفات هنا مطابقة لنفس أسماء الملفات اللي هترفعها على GitHub
    books_list = ["Clinical Pharmacology Made Incredibly Easy (3rd Ed.).pdf", "Book_2.pdf", "Book_3.pdf"]
...
def main():
    ...
    if not vector_store:
        st.error("❌ لم يتم العثور على الملفات الطبية. تأكد من رفع ملفات PDF في الريبو.")
        return

    tab1, tab2, tab3, tab4 = st.tabs([
        "👁️ الروشتات", "💬 استفسار طبي", "⚠️ التعارضات", "🩸 التحاليل"
    ])
        ...
        if up_file and st.button("🚀 تحليل الروشتة"):
            ...
            with st.spinner("👀 جاري القراءة..."):
                ...
                with st.spinner("🧠 جاري التحليل..."):
                    q = f"النص ده من روشتة مريض: '{raw_ocr_text}'. استنتج الأدوية الصح، استخداماتها، التشخيص، واقترح تحاليل طبية ضرورية لو الحالة تستدعي. اكتب تقرير منظم باللهجة المصرية."
    ...
    with tab3:
        ...
        if drugs_input and st.button("🧪 فحص التعارضات"):
            with st.spinner("🚨 جاري البحث..."):
                q = f"ابحث عن التفاعلات الدوائية بين: {drugs_input}. هل يوجد تعارض خطير؟ وما البديل؟ الرد باللهجة المصرية."
                ans = ask_smart_assistant(llm, vector_store, q)
    ...
    with tab4:
        ...
        lab_file = st.file_uploader("صورة التحليل", type=['jpg','png','jpeg'], key="lab")
        if lab_file and st.button("🧬 قراءة النتيجة"):
            ...
            with st.spinner("🔍 جاري التحليل..."):
                ...
                with st.spinner("💡 جاري اقتراح الأدوية..."):
                    q = f"هذا نص من نتيجة تحليل: '{lab_text}'. استخرج القيم غير الطبيعية، استنتج التشخيص، واقترح الأدوية العلمية المناسبة للحالة مع نصيحة للمريض باللهجة المصرية."
                    ans = ask_smart_assistant(llm, vector_store, q)
                    st.markdown(f"<div class='report-card'><h3>🩸 تقرير التحليل والعلاج:</h3>{ans}</div>", unsafe_allow_html=True)
